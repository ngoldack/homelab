...
x16 Tesla P100 and the Intel UHD 770 iGPU are passed through to the workers
that use them. Declared in `tofu/home/terraform.tfvars` (`proxmox_nodes`,
...
| `wk-main-performance` | home | performance (P) | 16 | 48 GiB | Tesla P100, nested KVM (Kata) | `workload/ai-inference`, `workload.hermes.io/sandbox` (explicitly untainted since 2026-09-15) |
...
threads on the P100 node. Fleet memory totals 82 GiB committed against the
...
- **Pod Security Admission.** Talos enforces `baseline` cluster-wide by
  ...
  `crowdsec`, `media`, `monitoring-crds`, `nvidia`, `tetragon`,
...
mounts or network policy. Kata microVMs pin to the P100 node by label.
...
| TrueNAS (`tank`/fast pools) | All PVs: CNPG databases, media/document libraries, registry, monitoring, llmkube cache/weights | Storage classes are `reclaimPolicy: Retain` and never delete by default | ZFS/TrueNAS-side rollback or clone — a NAS operation, not a Kubernetes one |
