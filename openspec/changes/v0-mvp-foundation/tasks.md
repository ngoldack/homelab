...
- [x] 1.1 Branch `v0-mvp-foundation`; rename `kubernetes/clusters/production` → `kubernetes/clusters/home` and fix Flux Kustomization `path:` references
...
## 2. tofu — single home node + GPU

- [x] 2.1 Define `node_pools` as 4 VMs on pmx-main (cp + wk + wk-gpu + wk-cpu), site=home labels; remove edge.tf / multi-site pools
...
- [x] 6.1 `apps/llama-cpp`: Deployment with the upstream CUDA `llama-server` image, `runtimeClassName: nvidia`, `nvidia.com/gpu: 1`, node pinned to `site=home`
...
- [x] 7.1 Full matrix: `kustomize build` ×(clusters/home, controllers, configs, apps) + `kubeconform -strict -ignore-missing-schemas` + `yamllint -c .yamllint` + `tofu validate`
