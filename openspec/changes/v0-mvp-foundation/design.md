...
The current cluster carries 30+ apps and several competing subsystems (vLLM, Ollama, SeaweedFS, SPIRE, Tetragon, Kyverno, NetBird, a research stack, home automation). For an MVP that proves "self-hosted LLM on home hardware," almost all of it is noise. v0 strips back to a single-node Talos cluster on `pmx-main` with the minimum needed to run and observe one model.

Hardware: Minisforum AR900i, 96GB DDR5, 1x Tesla P100 (Pascal, CC 6.0, 16GB HBM2). The home host is always Proxmox VE; Talos runs as a VM with the P100 passed through (never bare-metal). NVIDIA driver `580.159.04` is installed on the host and confirmed working with the P100.

## Goals / Non-Goals

**Goals:**
- One reproducible Talos node (control-plane + workloads) on the home site.
...
- **One cluster in v0 (built mesh-ready).** v0 is the home cluster only. It runs Cilium configured so Cluster Mesh can be turned on later without re-architecting. The **cloud** cluster (Hetzner VPS, id 2) + the actual Cluster Mesh join arrive in **v1**; **offsite** (TrueNAS VM, id 3) in **v3**. Separate meshed clusters (vs one stretched cluster) are the target because the links are WAN/untrusted and the cloud/offsite boxes are tiny.
...
- **Repo reduction is a deletion, not a rewrite.** Keep the proven Flux 3-tier layout; rename `clusters/production` → `clusters/home`; delete every out-of-scope app/controller in one sweep; `apps/` ends with just `llama-cpp`.
...
1. Branch. Rename `clusters/production` → `clusters/home`; update Flux Kustomization paths.
...
- **Topology:** home is always Proxmox VE; Talos runs as a VM with the P100 via PCIe passthrough (never bare-metal).
