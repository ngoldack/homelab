...
- **Four Talos nodes, all on the pmx-main Proxmox host (96GB / 32 threads):** a dedicated **control plane** (`cp`, 4GB, not schedulable), a **general worker** (`wk`, 12GB — cluster services / observability), a **GPU worker** (`wk-gpu`, 12GB + P100 passthrough — the chat model), and a **dedicated CPU worker** (`wk-cpu`, 64GB, tainted — the large coder model `qwen3-coder-next`). NVIDIA driver/toolkit + GPU kernel modules ship only on `wk-gpu` (per-pool), keeping the others clean. Cilium kube-proxy replacement makes this (and the future ai-host/cloud) trivial.
