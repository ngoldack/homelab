...
- **Two Talos nodes, both on the pmx-main Proxmox host:** (1) a control-plane node with the P100 (`allowSchedulingOnControlPlanes`, ~20GB) running the cluster + the GPU chat model, and (2) a **dedicated CPU-only inference node** (64GB, most cores, tainted) running a large coder model (`qwen3-coder-next`) via llama.cpp on CPU. The 96GB host is split 20/64 (~12GB host overhead). Cilium kube-proxy replacement makes the 2-node (and future ai-host/cloud) topology trivial.
