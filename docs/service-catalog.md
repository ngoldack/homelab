...
| `hermes-agents` (multi-agent: `orchestrator` Kanban dispatcher + `dave` WhatsApp/share/Matrix + `chad` coding/Matrix + `lindner` read-only finance/Matrix) | self | none public — each agent Matrix-bound to `matrix.ngoldack.de` (in-cluster); no HTTPRoute to the agents | per-agent PVCs 10 GiB (data: Matrix device store, Kanban board shared by `orchestrator`) + SOPS creds (Matrix tokens/devices/recovery keys, WhatsApp session, Hindsight tenant keys) | recovery keys + SOPS creds are the E2EE recovery path; the Kanban board + device store live on the agent PVCs | n/a | `hermes` (namespace/SAs/cilium), `matrix` (client API), `agentgateway` (`/v1/chain/*`), `hindsight` (per-tenant banks), `authentik` (issuer), `hermes-sandbox` (exec) |
