...
| `media` (jellyfin, jellyseerr, sabnzbd, sonarr, radarr, prowlarr) | self | LAN only — `*.media.svc.ngoldack.de` at VIP `.200` | library 4 TiB RWX `truenas-tank-nfs-media-library` + config PVCs on `truenas-fast-nfs-media-config` + CNPG `media-database` (1 instance; radarr/sonarr/prowlarr main+log DBs) on `truenas-fast-nvmeof-media-database` | CNPG Barman → S3 (`media/` prefix), retention 30d; library snapshot tasks `[unverified]` | DB RPO ≤60s; library `[unverified]` | Multus + VLAN 2080 VPN egress, CNPG, truenas-csi, authentik (jellyfin LDAP provider; *arr UIs behind the proxy outpost) |
