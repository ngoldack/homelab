...
- **One cluster in v0 (built mesh-ready).** v0 is the homelab cluster only. It runs Cilium configured so Cluster Mesh can be turned on later without re-architecting. The **cloud** cluster (Hetzner VPS, id 2) + the actual Cluster Mesh join arrive in **v1**; **offsite** (TrueNAS VM, id 3) in **v3**. Separate meshed clusters (vs one stretched cluster) are the target because the links are WAN/untrusted and the cloud/offsite boxes are tiny.
