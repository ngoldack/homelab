...
The system SHALL run kagent with a ModelConfig that points at the agentgateway `default` alias, so agents use llama.cpp without referencing a model.

#### Scenario: Agent completes a task
- **WHEN** a kagent agent is invoked
- **THEN** it reasons via agentgateway → llama.cpp and can complete a tool call
