...
- **Three clusters, not one (joined by Cilium Cluster Mesh).** v0 builds two: **homelab** (the 4-VM cluster above, id 1, all heavy compute) and **cloud** (one Hetzner VPS, id 2, single-node cp+workloads — the public ingress edge). **offsite** (id 3, one TrueNAS Scale VM) joins in v3. Each cluster runs its own Cilium + Flux and a subset of workloads; Cluster Mesh (unique id/name + shared CA + clustermesh-apiserver) gives cross-cluster service discovery so e.g. a cloud ingress route can target a homelab Service marked global. Separate clusters (vs one stretched cluster) because the links are WAN/untrusted and the cloud/offsite boxes are tiny — a stretched control plane over the internet is fragile; meshed independent clusters are not.
