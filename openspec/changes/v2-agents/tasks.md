...
- [ ] 1.2 Point n8n LLM/agent steps at the LiteLLM endpoint (`default` alias)
...
- [ ] 2.2 `apps/kagent`: a minimal agent set + a ModelConfig pointing at the LiteLLM `default` alias
...
- [ ] 4.2 LiteLLM gains the `embeddings` alias; mem0 uses it for embeddings and `default` for extraction
...
- [ ] 5.3 End-to-end: an n8n workflow and a kagent agent both produce a completion through LiteLLM → llama.cpp; mem0 recall works; agentgateway enforces auth
